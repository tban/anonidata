...
pub fn spawn_sidecar(app: &tauri::AppHandle) -> Result<BackendController, String> {
    let shell = app.shell();
                        ...
                        if !line.is_empty() {
                            if line.contains('{') && (line.contains("success") || line.contains("status")) {
                                if let Ok(json) = serde_json::from_str::<Value>(&line) {
                                    if json.get("status").and_then(|s| s.as_str()) == Some("progress") {
                                        let _ = app_handle.emit("backend-progress", &json);
                                    } else {
